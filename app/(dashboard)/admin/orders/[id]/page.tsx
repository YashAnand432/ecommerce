...
import { config, isValidEmailAddressFormat, isValidNameOrLastname } from "@/lib/utils";
...
const AdminSingleOrder = () => {
  ...
  useEffect(() => {
    const fetchOrderData = async () => {
      const response = await fetch(
        `${config.apiURL}/api/orders/${params?.id}`
      );
    ...
    const fetchOrderProducts = async () => {
      const response = await fetch(
        `${config.apiURL}/api/order-product/${params?.id}`
      );
  ...
  const updateOrder = async () => {
    if (
      ...
      fetch(`${config.apiURL}/api/orders/${order?.id}`, {
        method: "PUT", // or 'PUT'
        headers: {
  ...
  const deleteOrder = async () => {
    ...
    fetch(
      `${config.apiURL}/api/order-product/${order?.id}`,
      requestOptions
    ).then((response) => {
      fetch(
        `${config.apiURL}/api/orders/${order?.id}`,
        requestOptions
      ).then((response) => {
      ...
      <div className="flex flex-col gap-y-7 xl:ml-5 w-full max-xl:px-5">
        ...
        <div>
          ...
          <div className="flex gap-x-2 max-sm:flex-col mt-5">
            <button
              type="button"
              className="uppercase bg-gray-800 px-10 py-5 text-lg border border-black border-gray-300 font-bold text-white shadow-sm hover:bg-gray-900 hover:text-white focus:outline-none focus:ring-2"
              onClick={updateOrder}
            >
              Update order
            </button>
