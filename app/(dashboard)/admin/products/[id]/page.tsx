...
const DashboardProductDetails = ({
  ...
  const uploadFile = async (file: any) => {
    ...
    try {
      const response = await fetch(`${config.apiURL}/api/main-image`, {
        method: "POST",
        body: formData,
      });
  ...
  const fetchProductData = async () => {
    fetch(`${config.apiURL}/api/products/${id}`)
      .then((res) => {
        return res.json();
      })
      .then((data) => {
    ...
    const imagesData = await fetch(`${config.apiURL}/api/images/${id}`, {
      cache: "no-store",
    });
      ...
      <div className="flex flex-col gap-y-7 xl:ml-5 w-full max-xl:px-5">
        ...
        <div className="flex gap-x-2 max-sm:flex-col">
          <button
            ...
            className="uppercase bg-gray-900 px-10 py-5 text-lg border border-black border-gray-300 font-bold text-white shadow-sm hover:bg-blue-600 hover:text-white focus:outline-none focus:ring-2"
          >
            Update product
          </button>
          <button
